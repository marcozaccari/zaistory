...
Uso:
    ...
    python generate.py assets/out/metalhead/assets_manifest.json -o assets/out/metalhead
...
def load_profile(explicit=None) -> str | None:
    """Chiave dall'ambiente, o da `assets/.profile` se l'ambiente e' vuoto.
    ...
    accanto agli script (`assets/generator/../.profile`) e risalendo dalla
    ...
    candidates += [p / "assets" / PROFILE_NAME for p in (cwd, *cwd.parents)]
...
def main(argv=None):
    ...
    ap.add_argument("--key", help=f"default: ${ENV_KEY}, o assets/{PROFILE_NAME}")
    ...
    if not args.key and not args.dry_run:
        raise SystemExit(
            ...
            f"assets/{PROFILE_NAME} (gia' ignorato da git), oppure passa --key")
