...
export class Voce {
  ...
  private accoda(t: string): void {
    ...
    try {
      ...
      // Una sintesi che rifiuta di parlare non deve fermare la partita: il
      // giro dei turni passa di qui, e un'eccezione da qui uscirebbe dritta
      // nel gestore del tocco che stava facendo cominciare la storia.
      this.inCoda = Math.max(0, this.inCoda - 1);
      if (this.inCoda === 0) this.fineCoda();
    }
  }
