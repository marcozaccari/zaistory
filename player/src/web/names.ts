...
const NOMI: Record<string, string> = {
  // Copertina.
  zaistory_version: 'versione',
  generated_by: 'compilata da',
  id: 'identificativo',
  language: 'lingua',
  acts: 'atti',
  places: 'luoghi',
  start_act: 'primo atto',
  failure_mode: 'si può perdere',
...
/** Il tipo di fase, detto a chi gioca invece che allo schema. */
const TIPI_FASE: Record<string, string> = {
  interactive: 'da giocare',
  cutscene: 'da guardare',
};

export function nomeTipoFase(tipo: string): string {
  return TIPI_FASE[tipo] ?? tipo;
}
...
export function nomeCampo(campo: string): string {
  ...
  if (/^objects\./.test(campo)) return 'oggetto';
